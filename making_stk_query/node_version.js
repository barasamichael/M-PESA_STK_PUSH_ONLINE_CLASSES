 ...
 */
app.post('/query-stk-status', async (req, res) => {
    const checkoutRequestId = req.body.checkout_request_id;

    if (!checkoutRequestId) {
        return res.json({
            error: "Checkout Request ID not provided"
        });
    }
    ...
    try {
        ...
        const queryUrl = `${config.BASE_URL}${config.STK_QUERY_URL}`;
        const queryHeaders = {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
        };
